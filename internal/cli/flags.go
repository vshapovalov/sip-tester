...
func ParseArgs(args []string) (*config.Config, error) {
	...
	fs.StringVar(&cfg.HangupMode, "hangup-mode", "local", "hangup initiator: local|remote (remote waits up to 15s after RTP replay)")
	...
	fs.StringVar(&cfg.Transport, "transport", "udp", "SIP signaling transport: udp|tcp|tls (RTP always uses UDP)")
	fs.StringVar(&cfg.TLSCAFile, "tls-ca-file", "", "PEM CA certificates to trust in addition to system roots (TLS only)")
	fs.BoolVar(&cfg.TLSInsecure, "tls-insecure", false, "disable TLS certificate verification for testing (TLS only)")
