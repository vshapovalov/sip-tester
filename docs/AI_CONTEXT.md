...
9. **Bind RTP sockets and build SDP offer**
   - After local IP/family selection, bind two UDP sockets on `--local-ip` (audio + video).
   - Both local RTP ports are allocated from `10000-20000`.
   - New SDP is built with current local IP and those exact bound ports.
...
- `--ua`
  - SIP `User-Agent` header value for SIP requests and SIP responses generated by the tool.
  - Defaults to `sip-tester` when omitted.
...
- media ports are runtime bound RTP ports:
  - audio: first successfully bound socket port in `10000-20000`
  - video: second successfully bound socket port in `10000-20000`
